...
def run_pipeline(gt_col: str = 'gt', test_years: tuple = tuple(np.arange(2003, 2024, 2)),
    ...
    processed_data, processed_ground_truth = ml_preprocess_data(basic_preprocessed_df, test_years=test_years, load_from_cache=load_from_cache)
    ...
    for site_id in site_ids:
        ...
        test_site = test_features[test_features.site_id == site_id].drop(columns=non_feat_cols)
...
def train_val_test_split(feature_df: pd.DataFrame, gt_df: pd.DataFrame, test_years: tuple, validation_years: tuple):
    ...
    assert train_feature_df.date.isin(val_feature_df.date).sum() == 0 and \
           ...
           val_feature_df.date.isin(test_feature_df.date).sum() == 0, "Dates are overlapping between train, val, and test sets"

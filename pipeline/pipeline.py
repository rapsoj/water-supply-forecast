...
from models.fit_to_data import merge_global_local_models
...
def run_pipeline(test_years: tuple = tuple(np.arange(2005, 2024, 2)),
    ...
    local_df = run_local_models(train_features, val_features, test_features, train_gt, val_gt, gt_col, site_ids)

    final_df = merge_global_local_models(global_df, local_df)
...
def run_global_models(train_features, val_features, test_features, train_gt, val_gt, gt_col, site_ids):
    ...
    # todo perhaps find a better way of treating NaN values (american sites for volume+SNOTEL)
    train_features = train_features.fillna(0)
    val_features = val_features.fillna(0)
    test_features = test_features.fillna(0)

    train_pred, val_pred, test_pred = gen_basin_preds(train_features, train_gt[gt_col], val_features,
                                                      val_gt[gt_col], test_features)
    ...
    train_gt[gt_col] = train_gt[gt_col] * gt_std + gt_mean
    val_gt[gt_col] = val_gt[gt_col] * gt_std + gt_mean
    ...
    for site_id in site_ids:
        test_site_pred = test_pred[test_pred.site_id == site_id]
        cache_preds(pred=test_site_pred, cache_id=results_id, pred_dates=test_dates, site_id=site_id)

    ordered_site_ids = train_gt.site_id.drop_duplicates().tolist()
    print('Generating global model submission file...')
    return generate_submission_file(ordered_site_ids=ordered_site_ids, model_id='global')
